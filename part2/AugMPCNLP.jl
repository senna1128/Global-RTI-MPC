...
include("SelectEta.jl")
include("BackAug.jl")

# This function is the main body of Algorithm 1

function AugMPCNLP(N, MM, Ak, Bk, Ck, mu, eta1, eta2, beta, rho, eps, barX0, rep)
    # set random seed
    ...
    for rr = 1:rep
        ...
        while t <= N - MM
            ...
            # select eta1 and eta2
            eta1, eta2, InnerProd = SelectEta(MM, NewDir, Grad, H, G, res[end], eta1, eta2, rho)
            # backtracking line search to find stepsize and update
            X1, U1, Lambda1 = BackAug(MM,ak,bk,ck,X,U,Lambda,barX0,NewDir,Grad,Lag,mu,eta1,eta2,beta,InnerProd)
